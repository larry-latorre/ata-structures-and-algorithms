...
const addExclamation = (arr) => {
  const output = [];
  arr.forEach((item) => {
    const addExclamationOnItem = `${item}!`;
    output.push(addExclamationOnItem);
  });
...
const allUpperCase = (arr) => {
  const output = [];
  arr.forEach(item => {
    const stringsUpperCase = item.toUpperCase();
    output.push(stringsUpperCase);
...
const greeting = (word) => {
  ...
  const addExclamation = `${upperCaseString}!`;
