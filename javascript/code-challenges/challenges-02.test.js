...
const addQuestion = (arr) => {
  // Solution code here...
  const addQuestionMark = arr.map(word => word + '?');
...
const forLoopTwoToThe = (arr) => {
  ...
  const result = [];
  for (let num of arr){
    let poweredInt = Math.pow(2, num);
    result.push(poweredInt);
  }
  return result;
};
...
const forEachTwoToThe = (arr) => {
  ...
  const result = [];
  arr.forEach(number =>{
    const poweredInt = Math.pow(2,number);
    result.push(poweredInt);
  });
  return result;
};
