...
const removeThree = (idx, arr) => {
  ...
  if (idx >= 0 && idx < arr.length) {
    ...
    console.error('Invalid index. Index should be within the bounds of the array.');
  }
};
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    const firstSpaceIndex = recipe.ingredients[i].indexOf(' ');
    const foodName = recipe.ingredients[i].slice(firstSpaceIndex + 1);
